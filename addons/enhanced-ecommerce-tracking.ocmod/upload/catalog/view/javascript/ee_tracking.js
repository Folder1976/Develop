...
var ee_product = {
    'click': function (product_id, position, type) {
        $.ajax({
            url: 'index.php?route=module/ee_tracking/click',
            type: 'post',
            ...
            error: function(xhr, exc, error) {
                $.post('index.php?route=module/ee_tracking/clicklog',
...
var ee_promotion = {
    'click': function (banner_id, position) {
        $.ajax({
            url: 'index.php?route=module/ee_tracking/promotionclick',
            type: 'post',
            ...
            error: function(xhr, exc, error) {
                $.post('index.php?route=module/ee_tracking/promotionlog',
                    { 'error': error + ' (exc: ' + exc + ' status: ' + xhr.statusText + ')', 'url': window.location.href }, function( logs ) {
                        console.log(logs);
                    });
            }
...
$(document).ready(function() {
    $(document).on('click', '[onclick*=\'cart.add\']', function(e) {
        ...
        if (ee_data[0]) {
            $.ajax({
                url: 'index.php?route=module/ee_tracking/quickaddtocart',
                type: 'post',
                ...
                error: function(xhr, exc, error) {
                    $.post('index.php?route=module/ee_tracking/addtocartlog',
                        { 'error': error + ' (exc: ' + exc + ' status: ' + xhr.statusText + ')', 'url': window.location.href }, function( logs ) {
                            if (logs) {
                                console.log(logs);
                            }
    ...
    $('#button-cart').on('click', function() {
        $.ajax({
            url: 'index.php?route=module/ee_tracking/addtocart',
            type: 'post',
    ...
    if ($.inArray($.trim(document.location.pathname + document.location.search),['false']) !== -1) {
        setIntervalEE(function() {
            $.ajax({
                ...
                error: function(xhr, exc, error) {
                    $.post('index.php?route=module/ee_tracking/checkoutlog',
                        { 'error': error + ' (exc: ' + exc + ' status: ' + xhr.statusText + ')', 'url': window.location.href }, function( logs ) {
                            if (logs) {
                                console.log(logs);
                            }
