...
class ModelExtensionMultycurrgoods extends Model {
    ...
    public function set_custom_extra_charge_info($product_id, $data){
        ...
        $sp_to_res_price = $this->explode_price($data['sp_to_res_price']);
        $sp_min_max_res_price = $this->explode_price($data['sp_min_max_res_price']);
        $sp_from_res_price = $this->explode_price($data['sp_from_res_price']);
        $pop_to_res_price = $this->explode_price($data['pop_to_res_price']);
        $pop_min_max_res_price = $this->explode_price($data['pop_min_max_res_price']);
        $pop_from_res_price = $this->explode_price($data['pop_from_res_price']);

        if(count($query->rows) != 0){
            $this->db->query("UPDATE " . DB_PREFIX . "product_myltycurr_custom_extra_charge SET sp_from = '" . $this->db->escape($data['sp_from']) . "', sp_min_max = '" . $this->db->escape($data['sp_min_max']) . "', sp_to = '" . $this->db->escape($data['sp_to']) . "', pop_from = '" . $this->db->escape($data['pop_from']) . "', pop_min_max = '" . $data['pop_min_max'] . "', pop_to = '" . $data['pop_to'] . "', count_in_pack = '" . $data['boxing'] . "', sp_to_res_price = '" . $sp_to_res_price . "', sp_min_max_res_price = '" . $sp_min_max_res_price . "', sp_from_res_price = '" . $sp_from_res_price . "', pop_to_res_price = '" . $pop_to_res_price . "', pop_min_max_res_price = '" . $pop_min_max_res_price . "', pop_from_res_price = '" . $pop_from_res_price . "', active = '1' WHERE `product_id` = '" . (int)$product_id . "'");
        }
        else{
            $this->db->query("INSERT INTO " . DB_PREFIX . "product_myltycurr_custom_extra_charge SET product_id = '" . $this->db->escape($product_id) . "', sp_from = '" . $this->db->escape($data['sp_from']) . "', sp_min_max = '" . $this->db->escape($data['sp_min_max']) . "', sp_to = '" . $this->db->escape($data['sp_to']) . "', pop_from = '" . $this->db->escape($data['pop_from']) . "', pop_min_max = '" . $this->db->escape($data['pop_min_max']) . "', pop_to = '" . $this->db->escape($data['pop_to']) . "', count_in_pack = '" . $this->db->escape($data['boxing']) . "', sp_to_res_price = '" . $sp_to_res_price . "', sp_min_max_res_price = '" . $sp_min_max_res_price . "', sp_from_res_price = '" . $sp_from_res_price . "', pop_to_res_price = '" . $pop_to_res_price . "', pop_min_max_res_price = '" . $pop_min_max_res_price . "', pop_from_res_price = '" . $pop_from_res_price . "', active = '1'");
        }
...
//
        ...
        $result['sp_to_res']= (($input_iprice / (double)$currency_info['value']) * ($sp_to / 100)) + $input_iprice;
        $result['sp_min_max_res']= (($input_iprice / (double)$currency_info['value']) * ($sp_min_max / 100)) + $input_iprice;
        $result['sp_from_res']= (($input_iprice / (double)$currency_info['value']) * ($sp_from / 100)) + $input_iprice;
        $result['pop_to_res']= (($input_iprice / (double)$currency_info['value']) * ($pop_to / 100)) + $input_iprice;
        $result['pop_min_max_res']= (($input_iprice / (double)$currency_info['value']) * ($pop_min_max / 100)) + $input_iprice;
        $result['pop_from_res']= (($input_iprice / (double)$currency_info['value']) * ($pop_from / 100)) + $input_iprice;
    ...
    private function explode_price($val){
        $pieces = explode(" ", $val);
        $result = (double)$pieces[0];
        return $result;
    }
}