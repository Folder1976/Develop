...
class ControllerExtensionModuleRelatedOptions extends Controller {
  ...
  public function index() {
    ...
    $ro_language = $this->load->language('extension/module/related_options');
    ...
    $this->load->model('extension/module/related_options');
      ...
      if (isset($this->request->post['variants'])) {
        $this->model_extension_module_related_options->set_variants_options($this->request->post['variants']);
        unset($this->request->post['variants']);
      } else {
				$this->model_extension_module_related_options->set_variants_options(array());
			}
    ...
    if (isset($this->request->post['related_options'])) {
		...
		$current_version = $this->model_extension_module_related_options->current_version();
		if ( !isset($data['modules']['related_options_version']) || $data['modules']['related_options_version'] < $current_version
			...
			$this->model_extension_module_related_options->install_additional_tables();
		...
		$data['export_new_action'] 					= $this->url->link('extension/module/related_options/export_new', '&token=' . $this->session->data['token'], 'SSL');
    ...
    $data['options'] = $this->model_extension_module_related_options->get_compatible_options();
    $data['variants_options'] = $this->model_extension_module_related_options->get_variants_options();
			...
			if (count($data) > 1) {
				...
				if (!isset($json['error'])) {
					...
					if (isset($this->request->post['import_delete_before']) && $this->request->post['import_delete_before'] == 1) {
						$this->model_extension_module_related_options->delete_all_related_options();
					}
					...
					foreach ($products as $product_id => $product) {
						...
						if (isset($this->request->post['import_delete_before']) && $this->request->post['import_delete_before'] == 2) {
							$this->model_extension_module_related_options->delete_all_related_options($product_id);
						}
						
						
						$ro_data = $this->model_extension_module_related_options->get_ro_data($product_id);
						...
						$this->model_extension_module_related_options->set_ro_data($product_id, $product_data);	
    ...
    $this->model_extension_module_related_options->install();
		...
		$this->load->model('setting/setting');
		$msettings = array('related_options'=>array('update_quantity'=>1,'update_options'=>1,'ro_use_variants'=>1,'disable_all_options_variant'=>1,'related_options_version'=>$this->model_extension_module_related_options->current_version()));
  ...
  private function validate() {
    if (!$this->user->hasPermission('modify', 'extension/module/related_options') && !$this->user->hasPermission('modify', 'extension/extension/module/related_options')) {
