...
class ControllerCatalogProduct extends Controller {
	...
	protected function getList() {
      ...
      $data['ro_installed'] 										= $this->model_extension_module_related_options->installed();
			$data['variants_options'] 								= $this->model_extension_module_related_options->get_variants_options(true, true);
			$data['ro_all_options'] 									= $this->model_extension_module_related_options->get_compatible_options_values();
      ...
      $data['ro_installed'] 										= $this->model_extension_module_related_options->installed();
		...
		// << Related Options / Связанные опции 
			...
			if (isset($this->request->post['ro_data'])) {
				...
				$data['ro_data'] = $this->model_extension_module_related_options->get_ro_data($product_info['product_id']);
			} elseif ( isset($this->request->get['product_id']) && $this->request->get['product_id'] ) { // other module compatibility
				$data['ro_data'] = $this->model_extension_module_related_options->get_ro_data($this->request->get['product_id']);
			} else {
				$data['ro_data'] = array();
			}
				...
				foreach ($ro_data as $ro_dataset) {
					...
					if (isset($ro_dataset['ro']) && isset($ro_dataset['use']) && $ro_dataset['use'] ) {
						...
						if (is_array($ro)) {
							...
							$voptions = $this->model_extension_module_related_options->get_options_for_variant($ro_dataset['rov_id']);
